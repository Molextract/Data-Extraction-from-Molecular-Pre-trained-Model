...
class sac:
    ...
    def __init__(self, writer, args, env_fn, actor_critic=core.GCNActorCritic, ac_kwargs=dict(), seed=0,
        ...
        self.writer = writer
        # self.fname = 'molecule_gen/' + args.name_full + f"_action_{args.max_action}" + f"scaffold_{args.scaffold_idx}" + '.csv'
        self.root = f'molecule_gen/{args.name_full}'
        self.fname = f'molecule_gen/{args.name_full}/action_{args.max_action}_scaffold_{args.scaffold_idx}.csv'
        self.args = args
        # self.test_fname = 'molecule_gen/' + args.name_full + '_test.csv'
        self.save_name = './ckpt/' + args.name_full + f"_action_{args.max_action}" + f"scaffold_{args.scaffold_idx}" + '_'
    ...
    def train(self):
        ...
        # start_steps = 0
        start_steps = int(total_steps * 0.06)
        # update_after = int(total_steps * 0.06)
        update_after = int(total_steps * 0.02)
        ...
        for t in tqdm(range(total_steps)):
            ...
            # print("after:", Chem.MolToSmiles(self.env.mol))
            ...
            if t > 1 and t % self.docking_every == 0 and self.env.smile_list != []:
                ...
                if n_smi > 0:
                    # 计算score的地方
                    intr_rew_list = []
                    ext_rew = self.env.reward_GNN_batch()
                    for i in range(30):
                        if self.active_learning == "freed_pe":
                            ...
                            if not torch.isnan(loss_p).any():
                                ...
                                # print("Loss:", intr_rew.mean())
                                intr_rew_list.append(intr_rew.mean())
                            for e in self.emb_params:
                                e.requires_grad = True
                    # plt.plot([j for j in range(30)], intr_rew_list)
                    # plt.show()
                    ...
                    #         f.write(str)
                    # if self.steps_per_epoch // self.steps_per_epoch == self.epochs - 1:
                    if not os.path.exists(self.root):
                        os.makedirs(self.root)
                    if self.args.method == "thre":
                        self.fname = f'molecule_gen/{self.args.name_full}/threshold_action_{self.args.max_action}_scaffold_{self.args.scaffold_idx}_epoch{t}.csv'
                    else:
                        self.fname = f'molecule_gen/{self.args.name_full}/action_{self.args.max_action}_scaffold_{self.args.scaffold_idx}_epoch{t}.csv'
                        ...
                        for i in range(n_smi):
                            if self.args.method == "thre":
                                if ext_rew[i] > 0:
                                    ...
                                    str1 = f'{self.env.smile_list[i]} {num}' + '\n'
                                    f.write(str1)
                            else:
                                str1 = f'{self.env.smile_list[i]}' + '\n'
                                f.write(str1)
                    if self.writer:
