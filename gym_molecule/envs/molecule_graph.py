...
# from gym_molecule.envs.pretrained_models.
from gym_molecule.envs.env_utils_graph import *
from gym_molecule.envs.pretrained_models.GNN_simple import GNN_Vina
from gym_molecule.envs.pretrained_models.GNN_aux import GNN_Aux
...
def reward_GNN(smis, records, predictor, reward_min=0):
    reward = np.array(predictor.predict(smis, records))
    # reward = np.clip(reward, reward_min, None)
    return reward
...
class MoleculeEnv(gym.Env):
    ...
    def init(self, docking_config=dict(), params=dict(), data_type='zinc', ratios=dict(), reward_step_total=1,
             is_normalize=0, reward_type='crystal', reward_target=0.5, has_scaffold=False, has_feature=False,
             is_conditional=False, conditional='low', max_action=128, min_action=20, force_final=False, load_cpu=1,
             version="v1"):
        '''
        ...
        # init smi
        self.starting_smi = SCAFFOLD_VOCAB[params['scaffold_idx']]  # for hit (benzene ring)
        self.smi = self.starting_smi

        self.mol = Chem.MolFromSmiles(self.smi)
        self.smile_list = []
        self.smile_each_list = []
        self.my_actions = []
        self.record_list = []
        self.smile_old_list = []
        self.smile_list_all = []
        self.record_list_all = []
        ...
        # self.predictor = GNN_Aux_New(params)
        if version == 'v1':
            self.predictor = GNN_Aux(params)
        # elif version == 'v2':
        #     self.predictor = GNN_Aux_New(params)
        # if params['model'] == 'context_pred' and params['model_name'] is not None:
        #     self.predictor = GNN_Aux(params)
        # elif params['model'] == 'context_pred':
        #     self.predictor = GNN_Vina(params)
        # else:
    ...
    def reset_batch(self):
        self.smile_list_all.append(self.smile_list)
        self.record_list_all.append(self.record_list)
        self.smile_list = []
        self.record_list = []
    ...
    def reward_GNN_batch(self):
        reward = np.array(self.predictor.predict(self.smile_list, self.record_list))
        return reward
    ...
    def step(self, ac):
        ...
        if (self.counter >= self.max_action) or get_att_points(self.mol) == []:
            ...
            self.my_actions.append([copy.deepcopy(self.mol), ac])
        ...
        print("")
        print("old:", Chem.MolToSmiles(self.mol_old))
        print("new:", Chem.MolToSmiles(self.mol))
        ...
        if new:
            ...
            if get_att_points(self.mol) != []:
                ...
                self.record_list.append(self.my_actions)
                self.my_actions = []
